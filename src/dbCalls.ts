...
export function insertMigrationRecord (migrationInfo: Advance, totalPrep: number, totalProd: number) {
    const db = new Database("migrations_advance.db");
    const { total_in_preprod, total_in_production} = migrationInfo;

    console.log(`Pre: , ${total_in_preprod} + ${totalPrep} & Prod: , ${total_in_production} + ${totalProd} `);
